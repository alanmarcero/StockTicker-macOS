...
final class QuarterlyPanelViewModelTests: XCTestCase {
    ...
    func testEMAs_weekRows_aboveEMA() {
        ...
        XCTAssertTrue(vm.emaDayRows.isEmpty)
        XCTAssertEqual(vm.emaWeekRows.count, 1)
        XCTAssertEqual(vm.emaWeekRows[0].breakoutPercent!, 4.0, accuracy: 0.01)
    }
    ...
    func testEMAs_crossRows_multipleSymbols_filtersBelow3Weeks() {
        ...
        let emaEntries: [String: EMACacheEntry] = [
            "AAPL": EMACacheEntry(day: 200.0, week: 210.0, weekCrossoverWeeksBelow: 2, weekBelowCount: nil),
            "MSFT": EMACacheEntry(day: 380.0, week: nil, weekCrossoverWeeksBelow: nil, weekBelowCount: nil),
    ...
    // MARK: - Below 5W (Weeks Below EMA)

    func testEMAs_belowRows_showsBelowSymbols() {
        let vm = QuarterlyPanelViewModel()
    ...
    func testEMAs_belowRows_nilBelowCount_excluded() {
        let vm = QuarterlyPanelViewModel()
    ...
    func testEMAs_scannerOnlySymbols_appendedToDayRows() {
        let vm = QuarterlyPanelViewModel()
        ...
        XCTAssertEqual(vm.emaDayRows.count, 1)
        XCTAssertEqual(vm.emaDayRows[0].symbol, "NVDA")
        XCTAssertEqual(vm.emaDayRows[0].breakoutPercent!, 15.0, accuracy: 0.01)
        XCTAssertEqual(vm.emaDayRows[0].id, "NVDA-ema-day")
    }

    func testEMAs_scannerOnlySymbols_appendedToWeekRows() {
        let vm = QuarterlyPanelViewModel()
        let scannerData = ScannerEMAData(
            dayAbove: [],
        ...
        XCTAssertEqual(vm.emaWeekRows.count, 1)
        XCTAssertEqual(vm.emaWeekRows[0].symbol, "NVDA")
        XCTAssertEqual(vm.emaWeekRows[0].breakoutPercent!, 8.0, accuracy: 0.01)
    }

    func testEMAs_scannerOnlySymbols_appendedToCrossRows() {
        let vm = QuarterlyPanelViewModel()
        let scannerData = ScannerEMAData(
            dayAbove: [],
            weekAbove: [],
        ...
        XCTAssertEqual(vm.emaCrossRows.count, 1)
        XCTAssertEqual(vm.emaCrossRows[0].symbol, "NVDA")
        XCTAssertEqual(vm.emaCrossRows[0].breakoutPercent!, 7.0, accuracy: 0.01)
    }

    func testEMAs_scannerOnlySymbols_appendedToBelowRows() {
        let vm = QuarterlyPanelViewModel()
        let scannerData = ScannerEMAData(
            dayAbove: [],
            weekAbove: [],
    ...
    func testHasScannerData_withData_returnsTrue() {
        let vm = QuarterlyPanelViewModel()
        let scannerData = ScannerEMAData(dayAbove: [], weekAbove: [], crossovers: [], crossdowns: [], below: [], scanDate: "2026-03-07")
    ...
    func testScannerScanDate_withData_returnsDate() {
        let vm = QuarterlyPanelViewModel()
        let scannerData = ScannerEMAData(dayAbove: [], weekAbove: [], crossovers: [], crossdowns: [], below: [], scanDate: "2026-03-07")
        vm.update(watchlist: [], quarterInfos: testQuarters, data: QuarterlyPanelData(quotes: [:], quarterPrices: [:], scannerEMAData: scannerData))

        XCTAssertEqual(vm.scannerScanDate, "2026-03-07")
    }

    func testScannerScanDate_withoutData_returnsNil() {
        let vm = QuarterlyPanelViewModel()
        vm.update(watchlist: [], quarterInfos: testQuarters, data: QuarterlyPanelData(quotes: [:], quarterPrices: [:]))

        XCTAssertNil(vm.scannerScanDate)
    }
